...
void forward_compute(const std::vector<Tensor> &inputs, const Param &params,
    ...
    matmul_forward(inputs[0], inputs[1], outputs[0], inputs[0].shape()[0],
                   inputs[0].shape()[1], inputs[1].shape()[1],
                   params["scale"].as<float>());
}
...
void backward_compute(const std::vector<Tensor> &ograd_and_inputs, const Param &params,
    ...
    matmul_backward_lhs(ograd_and_inputs[2], ograd_and_inputs[0], igrads[0],
                        ograd_and_inputs[1].shape()[0], ograd_and_inputs[1].shape()[1],
                        ograd_and_inputs[2].shape()[1], params["scale"].as<float>());
    matmul_backward_rhs(ograd_and_inputs[1], ograd_and_inputs[0], igrads[1],
                        ograd_and_inputs[1].shape()[0], ograd_and_inputs[1].shape()[1],
                        ograd_and_inputs[2].shape()[1], params["scale"].as<float>());
}
